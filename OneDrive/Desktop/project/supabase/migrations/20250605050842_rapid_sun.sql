/*
  # Add question types and progress tracking
  ...
  2. Schema Updates
    - Add question_type, scale_max, and is_required to assessment_questions
    - Add options for multiple choice questions
    - Add progress tracking table
    - Update response table to handle different question types

  3. Security
    - Enable RLS for new tables
    - Add policies for question options and progress
*/

-- Add question types enum
...
ADD COLUMN question_type question_type NOT NULL DEFAULT 'rating',
ADD COLUMN scale_max integer CHECK (scale_max >= 2 AND scale_max <= 10),
...
ADD COLUMN selected_option_id uuid REFERENCES question_options(id) ON DELETE SET NULL,
