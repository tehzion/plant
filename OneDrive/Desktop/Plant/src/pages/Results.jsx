...
const Results = () => {
  ...
  const handleDownload = async () => {
    showToast(language === 'ms' ? 'Menjana PDF... Sila tunggu.' : 'Generating PDF... Please wait.', 'info', 10000);

    try {
      await generatePDFReport(scan, language, translations);
      showToast(language === 'ms' ? 'PDF Dimuat Turun' : 'PDF Downloaded', 'success');
    } catch (error) {
      console.error('Error generating PDF:', error);
      showToast(language === 'ms' ? 'Gagal menjana PDF' : 'Failed to generate PDF', 'error');
      handleDownloadText();
    }
  };
...
${t('pdf.generatedBy')}
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      showToast(language === 'ms' ? 'Pautan disalin!' : 'Link copied!', 'success');
    } catch (err) {
      ...
      try {
        ...
        showToast(language === 'ms' ? 'Gagal menyalin pautan' : 'Failed to copy link', 'error');
      }
