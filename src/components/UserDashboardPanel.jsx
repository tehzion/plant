...
import {
    ...
    Trash2, X, CheckCircle2, Leaf, BrainCircuit, Sparkles, Send
} from 'lucide-react';
...
const ACTIVITY_TYPES_CFG = [
    ...
    { value: 'prune',     label: '✂️ Prune',    key: 'actPrune',     chemical: false },
...
const UserDashboardPanel = () => {
    ...
    const ACTIVITY_TYPES = useMemo(() => ACTIVITY_TYPES_CFG.map(a => ({
        ...a,
        label: `${a.label.split(' ')[0]} ${t(`profile.${a.key}`) || a.label.split(' ')[1]}`
    })), [t]);
    ...
    const handleParseNL = async () => {
        if (!nlText.trim() || parsingNl) return;
        setParsingNl(true);
        try {
            const lang = localStorage.getItem('appLanguage') || 'en';
            const parsed = await parseNaturalLog(nlText, lang);
            
            let plotId = '';
            if (parsed.plotId) {
                const p = plots.find(plot => plot.name.toLowerCase().includes(parsed.plotId.toLowerCase()));
                if (p) plotId = p.id;
            }
            
            const typeLower = String(parsed.type).toLowerCase() || 'note';
            const validTypes = ['note', 'scout', 'spray', 'fertilize', 'prune', 'inspect', 'harvest', 'other'];
            const finalType = validTypes.includes(typeLower) ? typeLower : 'note';
            
            setNoteForm(f => ({
                ...f,
                activity_type: finalType,
                plot_id: plotId || f.plot_id,
                chemical_name: parsed.chemicalName || '',
                chemical_qty: parsed.quantity || '',
                kg_harvested: parsed.kg_harvested || '',
                note: parsed.notes || '',
            }));
            
            setNlText('');
        } catch (error) {
            console.error('Failed to parse logic:', error);
            alert(t('error.aiGenerationFailed') || 'Failed to parse text. Please check connection.');
        } finally {
            setParsingNl(false);
        }
    };
                    ...
                    <div className="udp-actions-grid">
                        {(() => {
                            ...
                            const standardActions = [
                                { icon: <ClipboardList size={22} />, label: t('nav.history') || 'History', color: 'history-icon', to: '/history', action: null },
                                { icon: <ShieldCheck size={22} />, label: 'myGAP', color: 'mygap-icon', to: '/mygap', action: null },
                                { icon: <BookOpen size={22} />, label: t('nav.encyclopedia') || 'Encyclopedia', color: 'enc-icon', to: '/encyclopedia', action: null },
                            ];
        ...
        <div>
            ...
            {addingPlot && (
                <form className="udp-inline-form" onSubmit={handleAddPlot}>
                    <input className="udp-input" placeholder={t('profile.plotName') || 'Plot / Farm Name'} value={plotForm.name} onChange={e => setPlotForm(p => ({ ...p, name: e.target.value }))} required />
                    <input className="udp-input" placeholder={t('profile.cropType') || 'Crop Type (e.g. Durian)'} value={plotForm.cropType} onChange={e => setPlotForm(p => ({ ...p, cropType: e.target.value }))} />
                    <div style={{ display: 'flex', gap: 8 }}>
                        <input className="udp-input" style={{ flex: 1 }} type="number" min="0" step="0.1" placeholder={t('profile.area') || 'Area'} value={plotForm.area} onChange={e => setPlotForm(p => ({ ...p, area: e.target.value }))} />
                        <select className="udp-input" style={{ flex: 1 }} value={plotForm.unit} onChange={e => setPlotForm(p => ({ ...p, unit: e.target.value }))}>
                            <option value="acres">{t('profile.acres') || 'acres'}</option>
                            <option value="hectares">{t('profile.hectares') || 'hectares'}</option>
                        </select>
                    </div>

                    {/* T6: Soil Health Toggle */}
                    <button
                        type="button"
                        style={{ background: 'none', border: '1px dashed #94a3b8', borderRadius: 8, padding: '8px 12px', fontSize: '0.8rem', color: '#475569', fontWeight: 600, cursor: 'pointer', textAlign: 'left' }}
                        onClick={() => setShowSoilFields(v => !v)}
                    >
                        {showSoilFields ? '▼' : '▶'} 🧪 {t('profile.advancedSoilData') || 'Advanced Soil Data'} (pH / NPK)
                    </button>
                    {showSoilFields && (
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                            <div>
                                <label className="udp-form-label">{t('profile.phLevel') || 'pH Level'} (0-14)</label>
                    ...
                    <button type="submit" className="udp-submit-btn" disabled={savingPlot}>
                        ...
                        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', paddingLeft: 4 }}>
                            {plot.soil_ph != null && (
                                <span style={{ fontSize: '0.7rem', fontWeight: 700, background: plot.soil_ph < 5.5 || plot.soil_ph > 7.5 ? '#fef3c7' : '#d1fae5', color: plot.soil_ph < 5.5 || plot.soil_ph > 7.5 ? '#d97706' : '#065f46', padding: '2px 8px', borderRadius: 20 }}>
                                    pH {plot.soil_ph} {plot.soil_ph < 5.5 ? `⚠️ ${t('profile.acidic') || 'Acidic'}` : plot.soil_ph > 7.5 ? `⚠️ ${t('profile.alkaline') || 'Alkaline'}` : `✓ ${t('profile.optimal') || 'Optimal'}`}
                                </span>
                            )}
        ...
        <div>
            ...
            {addingNote && (
                <form className="udp-inline-form" onSubmit={handleAddNote}>
                    {/* Smart AI input field */}
                    <div style={{ display: 'flex', gap: '8px', marginBottom: '8px', background: '#f5f3ff', border: '1px solid #ddd6fe', borderRadius: '8px', padding: '8px 12px', alignItems: 'center' }}>
                        <Sparkles size={16} color="#8b5cf6" />
                        <input 
                            className="udp-input" 
                            style={{ flex: 1, border: 'none', background: 'transparent', padding: '4px', boxShadow: 'none' }} 
                            placeholder={t('form.nlPlaceholder') || "Type naturally... e.g. 'Sprayed 20L Mancozeb on Plot A today'"}
                            value={nlText}
                        ...
                        <button 
                            ...
                            style={{ background: '#8b5cf6', color: 'white', border: 'none', borderRadius: '6px', padding: '6px 12px', fontSize: '0.75rem', fontWeight: 700, cursor: 'pointer', transition: 'background 0.15s' }}
                        >
                            {parsingNl ? '...' : <><Send size={12} style={{ display: 'inline', marginRight: 4 }} />{t('form.autoFill') || 'Auto-fill'}</>}
                        </button>
                    </div>
                    ...
                    <div className="udp-form-row">
                        <div style={{ flex: 1 }}>
                            <label className="udp-form-label">🌡 {t('profile.tempAM') || 'Temp (°C)'}</label>
                            <input
                    ...
                    {/* Notes free text & Expenses */}
                    <div className="udp-form-row">
                        <div style={{ flex: 2 }}>
                            <label className="udp-form-label">{t('profile.notes') || 'Notes / Remarks'}</label>
                            <textarea
                                className="udp-input"
                                rows={2}
                        ...
                        <div style={{ flex: 1.2 }}>
                            <label className="udp-form-label">{t('profile.expenseCategory') || 'Category'}</label>
                            <select
                                className="udp-input"
                    ...
                    {/* T4: Photo Attachment */}
                    <div>
                        <label className="udp-form-label">📷 {t('profile.photoAttachment') || 'Photo Attachment'} <span style={{ fontWeight: 400, opacity: 0.8 }}>{t('profile.photoOptional') || '(optional)'}</span></label>
                        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                            <label style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 8, background: '#f1f5f9', border: '1.5px dashed #cbd5e1', borderRadius: 8, padding: '10px 14px', cursor: 'pointer', fontSize: '0.8rem', color: '#475569', fontWeight: 600 }}>
                            ...
                            {noteForm.photo_base64 && (
                                <div style={{ position: 'relative' }}>
                                    <img src={noteForm.photo_base64} alt="preview" style={{ width: 60, height: 60, borderRadius: 8, objectFit: 'cover', border: '1px solid #e2e8f0' }} />
                                    <button
                                        type="button"
                                        onClick={() => setNoteForm(f => ({ ...f, photo_base64: '' }))}
                                        style={{ position: 'absolute', top: -6, right: -6, background: '#ef4444', color: 'white', border: 'none', borderRadius: '50%', width: 18, height: 18, cursor: 'pointer', fontSize: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', lineHeight: 1 }}
                                    >✕</button>
                                </div>
                            )}
                        </div>
            ...
            <style>{`
                .udp-form-label {
                    ...
                    color: var(--color-text-secondary); margin-bottom: 5px; text-transform: uppercase;
                }
                .udp-form-required { color: #ef4444; }
                .udp-form-row { display: flex; gap: var(--radius-sm); align-items: flex-start; }
                ...
                .udp-activity-chip {
                    padding: 6px var(--radius-md); border-radius: 999px;
                ...
                .udp-activity-chip:hover:not(.active) { border-color: var(--color-text-light); }

                /* Timing toggle */
                .udp-timing-toggle {
                    display: flex; border: 1.5px solid var(--color-border); border-radius: var(--radius-sm); overflow: hidden;
                }
                .udp-timing-btn {
                    flex: 1; padding: 9px var(--radius-sm);
                    border: none; border-right: 1px solid var(--color-border);
                    background: white; font-size: 0.78rem; font-weight: 700;
                ...
                .udp-note-top {
                    display: flex; align-items: center; gap: var(--radius-sm); flex-wrap: wrap;
                }
                .udp-note-badge {
                    display: inline-block; padding: 2px var(--radius-sm); border-radius: 999px;
                    font-size: 0.7rem; font-weight: 700;
                }
                .udp-note-plot { font-size: 0.72rem; color: var(--color-text-secondary); }
                .udp-note-date {
                    margin-left: auto; font-size: 0.7rem; font-weight: 700;
                    color: var(--color-text-light); text-transform: uppercase; white-space: nowrap;
                }
                .udp-note-chem {
                    font-size: 0.82rem; color: var(--color-text-primary);
                }
                .udp-note-chem strong { color: var(--color-text-primary); }
                .udp-note-env {
                    display: flex; gap: var(--radius-sm);
                    font-size: 0.75rem; color: var(--color-text-secondary);
                }
                .udp-note-text {
                    margin: 4px 0 0; font-size: 0.85rem; color: var(--color-text-primary);
        ...
        <div className="udp-container">
            ...
            <div className="udp-profile-card">
                ...
                <div className="udp-profile-info">
                    ...
                    <p className="udp-email">{email}</p>
                    <span className="udp-badge">
                        <ShieldCheck size={11} />
            ...
            <style>{`
                ...
                .udp-avatar {
                    width: 5var(--radius-sm); height: 5var(--radius-sm); border-radius: 50%;
                    background: var(--gradient-primary); color: white;
                    font-size: 1.4rem; font-weight: 800;
                    display: flex; align-items: center; justify-content: center;
                    flex-shrink: 0; box-shadow: 0 4px var(--radius-md) rgba(0,177,79,0.3);
                }
                .udp-profile-info { flex: 1; min-width: 0; }
                .udp-display-name {
                ...
                .udp-email {
                    font-size: 0.78rem; color: var(--color-text-secondary); margin: 0 0 var(--radius-sm);
                ...
                .udp-badge {
                    ...
                    padding: 2px var(--radius-sm); font-size: 0.68rem; font-weight: 700;
                }
                .udp-signout-mini {
                    position: absolute; top: var(--radius-lg); right: var(--radius-lg);
                    background: white; border: 1px solid #fecaca; color: #ef4444;
                    border-radius: var(--radius-sm); padding: 6px; cursor: pointer;
                    display: flex; align-items: center; justify-content: center;
                    transition: background 0.15s;
                }
                .udp-signout-mini:hover { background: #fef2f2; }
                ...
                .udp-action-icon.history-icon { background: linear-gradient(135deg,#60a5fa,#3b82f6); }
                .udp-action-icon.mygap-icon   { background: linear-gradient(135deg,#a78bfa,#7c3aed); }
                .udp-action-icon.enc-icon     { background: linear-gradient(135deg,#fb923c,#ea580c); }
