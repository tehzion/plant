        ...
        <div className="udp-overview">
            ...
            {activeAlerts.length > 0 ? (
                <div className="udp-section udp-section-alert">
                    <SectionHeader
                        ...
                        action={<button className="udp-see-all" onClick={() => setTab('reports')}>{label('common.seeAll', 'See all')}</button>}
                    />
            ...
            {!hasLoggedToday && (
                <div className="udp-task-banner" onClick={() => { setTab('notes'); setAddingNote(true); }}>
                    ...
                    <div className="udp-task-content">
                        <div className="udp-task-title">{label('profile.logDailyTask', 'Log a Daily Farm Task')}</div>
                        <div className="udp-task-desc">{label('profile.logDailyHint', 'Record spray, scouting, harvest, or note to strengthen traceability.')}</div>
                    </div>
            ...
            {complianceNudges.length > 0 && (
                <div className="udp-section" style={{ borderLeft: '4px solid #f59e0b' }}>
                    <SectionHeader icon={<CheckSquare size={15} color="#f59e0b" />} title={label('profile.gapCompliance', 'Compliance Roadmap')} />
                ...
                <div className="udp-section" style={{ background: assessingRisk ? '#fefce8' : '#fef2f2', borderColor: assessingRisk ? '#fef08a' : '#fecaca', padding: '16px', display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
                    ...
                    <div style={{ flex: 1 }}>
                        <h3 style={{ margin: '0 0 4px', fontSize: '0.9rem', color: assessingRisk ? '#854d0e' : '#991b1b', fontWeight: 800 }}>
                            {assessingRisk ? label('profile.analyzingRisk', 'Analyzing Farm Risk Profile...') : label('profile.riskActionRequired', 'Action Required: Imminent Risk Detected')}
                        </h3>
                        {!assessingRisk && predictiveRisk && (
                            <>
                                ...
                                {predictiveRisk.recommendedTreatment?.prefillAllowed !== false && predictiveRisk.recommendedTreatment && (
                                    ...
                                    >
                                        <Sparkles size={14} /> {label('profile.logSuggestedTreatment', 'Log Suggested Treatment')}
                                    </button>
                                )}
                            </>
                        )}
            ...
            <div className="udp-section">
                <SectionHeader
                    ...
                    action={(
                        ...
                        >
                            {generatingInsights ? label('common.analyzing', 'Analyzing...') : <><Sparkles size={13} /> {label('profile.askAI', 'Ask AI')}</>}
                        </button>
                    )}
                />
                    ...
                    {generatingInsights ? (
                        ...
                        <div style={{ background: '#f5f3ff', border: '1px solid #ddd6fe', borderRadius: '8px', padding: '16px' }}>
                            <p style={{ fontSize: '0.85rem', color: '#4c1d95', margin: '0 0 12px', lineHeight: 1.5 }}>
                                <strong>{label('profile.aiSummary', 'Summary')}:</strong> {aiCardData.summary}
                            </p>
                            ...
                            <div style={{ fontSize: '0.75rem', fontWeight: 700, color: '#6d28d9', marginBottom: '8px', textTransform: 'uppercase' }}>
                                {label('profile.aiRecommendations', 'Actionable Recommendations')}:
                            </div>
                        ...
                        <div style={{ textAlign: 'center', padding: '16px', color: '#64748b', fontSize: '0.85rem' }}>
                            {label('profile.aiAskHint', 'Tap "Ask AI" to get a personalized weekly agronomist report based on your farm activity.')}
                        </div>
                    )}
                </div>
            </div>
                                    ...
                                    <div key={day.date || index} className="udp-forecast-card" style={{ background: notice.bg, borderColor: notice.border }}>
                                        <span className="udp-forecast-day">
                                            {index === 0
                                                ...
                                                : new Date(day.date).toLocaleDateString(label('common.dateLocale', 'en-MY'), { weekday: 'short' })}
                                        </span>
            ...
            <div className="udp-explore-grid">
                <div className="udp-explore-card" style={{ background: 'linear-gradient(135deg, #fff, #f0fdf4)' }} onClick={() => navigate('/mygap')}>
                    ...
                    <span className="udp-explore-label">{label('home.mygapTitle', 'myGAP Guide')}</span>
                    ...
                    <span style={{ fontSize: '0.65rem', fontWeight: 700, color: '#15803d' }}>{checklistPct}% {label('common.ready', 'Ready')}</span>
                </div>
            ...
            <div className="udp-section">
                <SectionHeader icon={<Calendar size={15} />} title={label('profile.recentScans', 'Recent Scans')} action={<button className="udp-see-all" onClick={() => setTab('reports')}>{label('common.seeAll', 'See all')}</button>} />
