...
const MyGapPage = () => {
    ...
    const generateReport = () => {
        ...
        doc.text(t('mygap.reportTitle'), marginLeft, yPos);
        yPos += 15;

        // Subtitle & Date
        doc.setFontSize(12);
        doc.setTextColor(100);
        doc.text(`${t('mygap.reportGenerated')}: ${new Date().toLocaleDateString()}`, marginLeft, yPos);
        yPos += 20;

        // Checklist Section
        doc.setFontSize(16);
        doc.setTextColor(0);
        doc.text(t('mygap.checklistTitle'), marginLeft, yPos);
        yPos += 10;

        doc.setFontSize(12);
        checklistItems.forEach(item => {
            const status = checklist[item.id] ? '[x]' : '[ ]';
            doc.text(`${status} ${item.label}`, marginLeft, yPos);
            yPos += 8;
        });
        yPos += 10;
        ...
        doc.setFontSize(16);
        doc.text(t('mygap.logbookSummary'), marginLeft, yPos);
        yPos += 10;

        if (logs.length === 0) {
            doc.setFontSize(12);
            doc.text(t('mygap.noLogs'), marginLeft, yPos);
        } else {
            doc.setFontSize(10);
            logs.slice(0, 10).forEach(log => {
                const date = new Date(log.timestamp).toLocaleDateString();
                const type = t(`mygap.${log.type}Log`);
                const note = log.notes.substring(0, 40) + (log.notes.length > 40 ? '...' : '');
                doc.text(`${date} - ${type}: ${note}`, marginLeft, yPos);
                yPos += 7;
            });
        }
        ...
        doc.text('Generated by Smart Plant Advisor', marginLeft, 280);
